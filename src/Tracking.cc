    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    if(settings){cout << "TestForDebug:" << "Use New ParameterLoader" << endl;
        newParameterLoader(settings);
    }
    else{cout << "TestForDebug:" << "Use Old ParameterLoader" << endl;
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    {
        //GCN
        mCurrentFrame = Frame(mImGray,mImDepth,timestamp,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    }
...
//双目初始化&RGBD初始化(两者共用一套初始化函数)
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)//If current number of the keypoints is greater than 500
    {   //如果用了IMU
